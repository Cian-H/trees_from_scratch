...
(println "\n--- Hybrid Oblique CART ---")
(println "Fitting Hybrid Oblique tree (Regression)...")
(def obl-tree (obl/train train-dataset target {:task-type :regression :num-candidates 100 :symmetry-threshold 1000.0}))
...
(println (format "Hybrid Oblique Training R^2: %.4f" obl-train-r2))
(println (format "Hybrid Oblique Testing R^2:  %.4f" obl-test-r2))
(println (format "Hybrid Oblique Tree Depth: %d" (tree-core/tree-depth obl-tree)))
