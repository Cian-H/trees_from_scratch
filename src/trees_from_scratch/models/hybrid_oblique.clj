(ns trees-from-scratch.models.hybrid-oblique
  "Implementation of Hybrid Oblique Regression Trees using Successive Halving and Random Projection symmetry breaking."
...
(defn best-split
  ...
  ([dataset target-key {:keys [features loss-fn num-candidates symmetry-threshold rng]
                        :or {features nil
   ...
   (let [available-features (or features (remove #{target-key} (ds/column-names dataset)))
     ...
     (if (and symmetry-threshold
              (or (nil? best-overall) (< (:loss-reduction best-overall) symmetry-threshold))
              (seq cont-features))
       (let [rp-split (rp/random-oblique-split dataset cont-features target-key (or rng (java.util.Random.)))]
         (if rp-split
           (if best-overall
             (if (> (:loss-reduction rp-split) (:loss-reduction best-overall)) rp-split best-overall)
             rp-split)
           best-overall))
       best-overall))))

(defn train
  "Trains a Hybrid Oblique Regression tree on the given dataset."
  ([dataset target-key]
   (train dataset target-key {}))

  ([dataset target-key {:keys [task-type stop loss-fn num-candidates symmetry-threshold]
                        :or   {task-type :regression
                               stop      stopping/cart-stopping-strategy
                               num-candidates 50}
                        :as   opt}]
   (if (not= task-type :regression)
     (throw (ex-info "Hybrid Oblique trees currently only support regression" {:task-type task-type}))
     (let [rng         (java.util.Random. (hash dataset))
           ...
           best-split-fn (fn [ds tgt o]
                           (let [feats (or (:features o) (remove #{tgt} (ds/column-names ds)))
                             ...
                             (best-split ds tgt (assoc o :features sampled :loss-fn loss-fn :num-candidates (or num-candidates 50) :symmetry-threshold symmetry-threshold :rng rng))))]
